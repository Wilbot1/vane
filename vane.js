...
async function starts() {
	...
	console.log('>', '[',color('INFO','blue'),']','Gracias por usar el bot :)..')
	turbo.on('qr', () => {
...
//Welkom
			...
			teks =  `sendButLocation(from, 𝘏𝘰𝘭𝘢 @${num.split('@')[0]}\n𝘉𝘪𝘦𝘯𝘷𝘦𝘯𝘪𝘥𝘰 𝘢𝘭 𝘨𝘳𝘶𝘱𝘰\n *${mdata.subject}*\n`
			...
			teks = `sendButLocation(from, 𝘈𝘥𝘪𝘰𝘴 @${num.split('@')[0]}\n  𝘌𝘴𝘱𝘦𝘳𝘰 𝘲𝘶𝘦 𝘵𝘦 𝘮𝘶𝘦𝘳𝘢𝘴 :𝘷\n
			let buff = await getBuffer(ppimg)
			turbo.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
...
turbo.on('chat-update', async (mek) => {
	try {
		...
		const ownerNumber = ['51985761261@s.whatsapp.net'] //turbo owner
//SETTING WeA
...
case prefix+ 'avatar':
		...
		case prefix+ 'waifu':
			...
			turbo.sendMessage(from, nyed, image, { caption: '𝐁𝐎𝐓𒇺𝐋𝐄𝐍𝐥', quoted: mek })
			.catch(err => {
				return('Ocurrió un error, intentalo de nuevo...')
			})
			break
...
break
	...
	case prefix+ 'husbu':
			...
			turbo.sendMessage(from, nye, image, { caption: '❮ ❮ 𝐁𝐎𝐓ꦼ𝐋𝐄𝐍𝐥 ❯❯', quoted: mek })
			.catch(err => {
				return('Ocurrió un error, intentalo de nuevo...')
			})
			break
...
case prefix+ 'pinterest':
			...
			ke =  sasu[Math.floor(Math.random() * sasu.length)];
			nye = await getBuffer(ke)
			turbo.sendMessage(from, nye, image, { caption: '𝐁𝐎𝐓𒇺𝐋𝐄𝐍𝐥', quoted: mek })
			.catch(err => {
				return('y el nombre?')
			})
			break
...
case prefix+ 'getbio':
	   ...
	   case prefix+ 'getpic':
		...
		turbo.sendMessage(from, thumb, MessageType.image, {caption: '𝐁𝐎𝐓𒇺𝐋𝐄𝐍𝐥'})
	        }
		break
case prefix+ 'setbio':
	        if (!isMe) return reply('Esté comando solo puede usado por mi')
...
case prefix+  'stats':
                ...
                await wa.sendFakeStatus(from, texxt, "𝐁𝐎𝐓𒇺𝐋𝐄𝐍𝐥")
                break
case prefix+ 'block':
...
//CONVERTER
    ...
    case prefix+ 'tovideo':
            ...
            sendMediaURL(from,res.result,'Wily uwu')
            })
            }else {
